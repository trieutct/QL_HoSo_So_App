                    ...
                    <el-radio-group v-model="typeTable" style="margin-bottom: 30px">
                        <el-tooltip class="box-item" effect="dark" content="Bố cục kiểu danh sách" placement="bottom">
                            <el-radio-button value="table" size="large">
                                <template #default>
                                    <i class="ri-check-line" v-if="typeTable === 'table'"></i> <i
                        ...
                        <el-tooltip class="box-item" effect="dark" content="Bố cục kiểu lưới" placement="bottom">
                            <el-radio-button value="grid" size="large">
                                <template #default>
                                    <i class="ri-check-line" v-if="typeTable === 'grid'"></i> <i
                                        class="ri-layout-grid-2-fill"></i>
                                </template>
                            </el-radio-button>
                        </el-tooltip>
                    </el-radio-group>
            ...
            <el-table :data="docsData" style="width: 100%">
                <el-table-column prop="docCode" label="Tên file" width="400">
                    <template #default="scope">
                        <i v-if="getFileInfoFromPath(scope.row.urlFile).extension === 'docx' || getFileInfoFromPath(scope.row.urlFile).extension === 'doc'"
                ...
                <el-table-column prop="issuedDate" label="Năm" width="150">
                    <template #default="scope">
                        {{ scope.row.issuedDate != "undefined" ? formatDateString(scope.row.issuedDate, "DD/MM/YYYY") :
                            "" }}
                    </template>
                </el-table-column>
                            ...
                            <el-dropdown trigger="click">
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu class="custom-dropdown">
                                        <el-dropdown-item @click="showFile(scope.row.urlFile)"><i
                                                class="ri-eye-line"></i>
                                            <p class="ml-5">Xem</p>
                                        </el-dropdown-item>
                                        <el-dropdown-item
                                            @click="dowloadFile(getFileInfoFromPath(scope.row.urlFile).fileName)">
                                        ...
                                        <el-dropdown-item @click="{ showDialogDelete = true; idDelete = scope.row.id }">
                                            <el-icon>
                                                <Delete />
                                            </el-icon>
                                            <p class="ml-5">Xóa</p>
                ...
                <el-col :span="6" class="mt-4" v-for="(doc, index) in docsData" :key="index">
                    <div class="w-full p-3 h-[200px] rounded-xl" style="background-color: #f0f4f9">
                        <div class="flex items-center justify-between">
                            <div>
                                <i v-if="getFileInfoFromPath(doc.urlFile).extension === 'docx' || getFileInfoFromPath(doc.urlFile).extension === 'doc'"
                                    class="ri-article-line text-blue-600 text-xl"></i>
                                <i v-if="getFileInfoFromPath(doc.urlFile).extension === 'pdf'"
                                    class="ri-file-pdf-line text-orange-500 text-xl"></i>
                            </div>
                            <div class="w-[70%] truncate">
                                <p class="text-sm">{{ getFileInfoFromPath(doc.urlFile).fileName }}</p>
                            </div>
                            <div>
                                <el-tooltip class="box-item" effect="dark" content="Thao tác" placement="top">
                                    <el-dropdown trigger="click">
                                        ...
                                        <template #dropdown>
                                            <el-dropdown-menu class="custom-dropdown">
                                                <el-dropdown-item @click="showFile(doc.urlFile)"><i
                                                        class="ri-eye-line"></i>
                                                    <p class="ml-5">Xem</p>
                                                </el-dropdown-item>
                                                <el-dropdown-item
                                                    @click="dowloadFile(getFileInfoFromPath(doc.urlFile).fileName)">
                                                    <i class="ri-file-download-line"></i>
                                                    <p class="ml-5">Tải xuống</p>
                                                </el-dropdown-item>
                                                ...
                                                <el-dropdown-item
                                                    @click="{ showDialogDelete = true; idDelete = doc.id }">
                                                    <el-icon>
                                                        <Delete />
                                                    </el-icon>
                                                    <p class="ml-5">Xóa</p>
                        ...
                        <div class="flex justify-center mt-2 borde">
                            <img :src="getFileInfoFromPath(doc.urlFile).extension === 'pdf' ? PDFIMAGE : WORDIMAGE"
                                alt="" class="rounded-lg mb-1"
                                :class="getFileInfoFromPath(doc.urlFile).extension === 'pdf' ? 'max-h-[140px] min-w-[200px]' : 'max-h-[140px]'">
                        </div>
                    </div>
                </el-col>
            </el-row>
