<template>
    ...
    <div class="w-full mt-5">
        <el-collapse class="w-full flex flex-wrap">
            <el-collapse-item v-model="list" v-for="i in moudle_operations" :key="i" class="w-1/4" :title="i.name" :name="i">
                <el-select multiple filterable allow-create default-first-option
                    :reserve-keyword="false" placeholder="Chọn" style="width: 240px">
                    <el-option @click="addValue(item.id)" v-for="item in i.operations" :key="item.id" :label="item.name" :value="item.id" />
                </el-select>
            </el-collapse-item>
        </el-collapse>
    </div>
...
const load=async()=>{
    ...
    console.log(moudle_operations.value)
}
onMounted(() => {
    load()
})
