<template>
    <h2 class="text-main font-semibold text-xl">Quản lý loại hồ sơ</h2>
    <div class="flex mt-5">
    ...
    <div class="custom-table mt-8">
        ...
        <el-table ref="multipleTableRef" @selection-change="handleSelectionChange" v-loading="loading.isLoading"
            :height="'calc(100vh - 220px)'" :data="loaiHosSos" border width="100%">
            <!-- <el-table  v-loading="loading.isLoading" :height="'calc(100vh - 220px)'" :data="loaiHosSos" border width="100%"> -->
            <el-table-column fixed type="selection" width="55" />
            <el-table-column prop="id" label="ID" width="100" />
            <el-table-column prop="name" label="Tên loại hồ sơ" width="200" />
            <el-table-column prop="code" label="Mã loại hồ sơ" width="200" />
...
import { useLoaiHoSo } from "../loaihoso";
...
const { query, getDataLoaiHoSo, loaiHosSos } = useLoaiHoSo();
...
const loadData = async () => {
    ...
    const res: any = await getDataLoaiHoSo();
    if (res.data) {
        loaiHosSos.value = res.data;
...
const deleteKho = async (id: string) => {
    try {
        const res: any = await loaiHoSoHoSoServiceApi._delete(id);
        if (res.success) {
...
const handleSelectionChange = (val: ILoaiHoSo[]) => {
    multipleSelection.value = val.map((item) => item.id);
};

const deleteMultiple = async (list_id: any) => {
    try {
        const res: any = await loaiHoSoHoSoServiceApi._softDeleteMutiple(list_id);
        if (res.success) {
            showSuccessNotification(res.message);
            loadData();
        } else {
