<template>
    <el-dialog style="border-radius: 5px !important"
        :title="props.itemEdit ? 'Cập nhật loại văn bản' : 'Tạo mới loại văn bản'" width="700">
        <el-row :gutter="20">
            <el-col :span="24">
                <p>
                    Mã loại hồ sơ
            ...
            <el-col class="mt-4" :span="24">
                ...
                <el-input v-model="name" size="large" style="width: 100%" placeholder="Tên loại văn bản" />
...
const getKhoById = async (id) => {
    try {
        ...
        const data = await loaiVanBanServiceApi._getDetail(id);
        if (data.success) {
...
const submit = handleSubmit(async () => {
    try {
        if (props.itemEdit === null) {
            ...
            loading.setLoading(true);
            const res = await loaiVanBanServiceApi.create(formData);
            if (res.success) {
                emits("close");
                emits("loadData");
            ...
            formData.append("Name", name.value);
            loading.setLoading(true);
            const res = await loaiVanBanServiceApi.update(props.itemEdit, formData);
            if (res.success) {
                emits("close");
                emits("loadData");
