<template>
    <h2 class="text-main font-semibold text-xl">Quản lý loại văn bản</h2>
    <div class="flex mt-5">
    ...
    <div class="custom-table mt-8">
        ...
        <el-table ref="multipleTableRef" @selection-change="handleSelectionChange" v-loading="loading.isLoading"
            :height="'calc(100vh - 220px)'" :data="loaiVanBans" border width="100%">
            <!-- <el-table  v-loading="loading.isLoading" :height="'calc(100vh - 220px)'" :data="loaiVanBans" border width="100%"> -->
            <el-table-column fixed type="selection" width="55" />
            <el-table-column prop="id" label="ID" width="100" />
            <el-table-column prop="name" label="Tên loại văn bản" width="200" />
            <el-table-column prop="code" label="Mã loại văn bản" width="200" />
...
import { useLoaiVanBan } from "../loaivanban";
...
const { query, getDataLoaiVanBan, loaiVanBans } = useLoaiVanBan();
...
const loadData = async () => {
    ...
    const res: any = await getDataLoaiVanBan();
    if (res.data) {
...
const deleteKho = async (id: string) => {
    try {
        const res: any = await loaiVanBanServiceApi._delete(id);
        if (res.success) {
...
const handleSelectionChange = (val: ILoaiVanBan[]) => {
    multipleSelection.value = val.map((item) => item.id);
};

const deleteMultiple = async (list_id: any) => {
    try {
        const res: any = await loaiVanBanServiceApi._softDeleteMutiple(list_id);
        if (res.success) {
            showSuccessNotification(res.message);
            loadData();
        } else {
